...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200">
          ...
          <div className="flex items-center justify-between h-20">
            ...
            <div className="flex-shrink-0">
              <a href="#" className="text-2xl font-black tracking-tighter text-slate-900 flex items-center gap-1">
                <span className="text-blue-900">SANGEETHA</span>
                <span className="text-amber-500 font-medium tracking-normal text-xl">LABEL</span>
              </a>
            </div>

            {/* Desktop Menu */}
            <div className="hidden md:flex space-x-6 items-center font-semibold text-sm text-slate-600 tracking-wide">
              <a href="#categories" className="hover:text-blue-900 transition-colors">Woven Labels</a>
              <a href="#categories" className="hover:text-blue-900 transition-colors">Heat Fusing</a>
              <a href="#categories" className="hover:text-blue-900 transition-colors">Laser Cut</a>
              <a href="#categories" className="hover:text-blue-900 transition-colors">Woven Badges</a>
              <a href="#products" className="hover:text-blue-900 transition-colors">Shop Stock</a>
              <a href="#quote" className="bg-blue-900 hover:bg-blue-950 text-white px-5 py-2.5 rounded-xl transition-colors text-xs uppercase tracking-widest font-bold shadow-md shadow-blue-900/10">
                Get A Quote
              </a>
            </div>
            ...
            <div className="md:hidden">
              <button 
                ...
                className="text-slate-700 hover:text-blue-900 p-2 focus:outline-none"
              >
        ...
        {mobileMenuOpen && (
          <div className="md:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-3 shadow-lg">
            ...
            <a href="#quote" onClick={() => setMobileMenuOpen(false)} className="block text-center bg-blue-900 text-white py-3 rounded-xl font-bold text-xs uppercase tracking-widest">
              Get A Quote
            </a>
          </div>
        )}
      ...
      {/* Hero Section */}
      <header className="relative bg-gradient-to-b from-blue-50/50 via-white to-white overflow-hidden py-20 lg:py-32 border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7 text-center lg:text-left">
            <h1 className="text-4xl sm:text-6xl font-black text-slate-900 tracking-tight leading-tight mb-6">
              Quality Custom Clothing <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-900 to-indigo-950 block sm:inline">Labels & Tags.</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <a href="#quote" className="bg-blue-900 text-white px-8 py-4 rounded-xl hover:bg-blue-950 font-bold tracking-wide text-sm shadow-xl shadow-blue-900/20 text-center transition-all hover:-translate-y-0.5">
                GET A CUSTOM QUOTE
              </a>
              <a href="#products" className="bg-white text-slate-800 border-2 border-slate-200 px-8 py-4 rounded-xl hover:bg-slate-50 font-bold tracking-wide text-sm text-center transition-all">
                ORDER STOCK LABELS
              </a>
            </div>
          </div>
          ...
          <div className="lg:col-span-5 relative flex justify-center items-center">
            ...
            <div className="relative w-72 h-72 sm:w-80 sm:h-80 bg-stone-100 rounded-2xl border-t-4 border-amber-500 p-6 shadow-2xl flex flex-col justify-between transform -rotate-3 hover:rotate-0 transition-transform duration-500">
              ...
              <div className="absolute -z-10 top-6 left-8 w-full h-full bg-blue-950 text-white rounded-2xl p-6 shadow-xl flex flex-col justify-between transform rotate-6 scale-95 opacity-95 border-b-4 border-amber-600">
      ...
      <section id="categories" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <p className="text-slate-600">Every single project receives dedicated architectural setups and robust vector mapping profiles.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {categories.map((cat, idx) => (
            <div key={idx} className="bg-white p-8 rounded-2xl border border-slate-200/60 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 group">
              <div className="text-4xl mb-6 group-hover:scale-110 transition-transform inline-block bg-blue-50 p-3 rounded-xl">{cat.image}</div>
              <h3 className="text-lg font-bold text-slate-900 mb-2 group-hover:text-blue-900 transition-colors">{cat.title}</h3>
      ...
      <section className="bg-slate-900 text-white py-24 border-y border-slate-800 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16">
            <span className="text-amber-500 font-bold uppercase text-xs tracking-widest block mb-2">Production Blueprint</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 relative">
            {orderSteps.map((step, idx) => (
              <div key={idx} className="bg-slate-950/50 border border-slate-800 rounded-2xl p-6 relative flex flex-col justify-between group hover:border-amber-500/40 transition-all duration-300 text-center">
                <div>
                  {/* Clean Circular Wrapper mirroring standard logo bar frames */}
                  <div className="w-32 h-32 mx-auto mb-6 bg-white rounded-full border-2 border-slate-800 flex items-center justify-center overflow-hidden p-1 group-hover:border-amber-500/40 transition-colors shadow-lg">
                    <img 
                      src={step.imgUrl} 
                      alt={step.altText} 
                  ...
                  <div className="mb-2">
                    <span className="text-xs font-mono font-bold text-amber-500 tracking-widest block mb-1">STAGE {step.num}</span>
                    <h3 className="text-lg font-bold text-white group-hover:text-amber-400 transition-colors">{step.title}</h3>
                  </div>
      ...
      <section id="products" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-12 gap-4">
          <div>
            ...
            <p className="text-slate-600 mt-1">Pre-folded, fully starched, and prepared for dispatch lines.</p>
          </div>
          <button className="text-sm font-bold text-blue-900 underline underline-offset-4 hover:text-blue-700">View All Stock ({popularProducts.length})</button>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {popularProducts.map((prod) => (
            <div key={prod.id} className="bg-white rounded-2xl overflow-hidden border border-slate-200/60 shadow-sm flex flex-col justify-between p-6 hover:border-blue-900/30 transition-all">
              <div>
                <div className="w-full aspect-square bg-slate-50 rounded-xl mb-4 flex items-center justify-center text-5xl relative border border-slate-100">
                  🏷️
                  {prod.badge && (
                    <span className="absolute top-2 right-2 bg-blue-900 text-white text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-md">
                      {prod.badge}
                    </span>
                  )}
                </div>
                <h3 className="font-bold text-slate-900 text-base mb-1 line-clamp-1">{prod.name}</h3>
                <p className="text-slate-500 text-xs mb-4">{prod.desc}</p>
              </div>
              <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                <span className="font-mono font-bold text-blue-900">
                  {prod.price === "0.00" ? "FREE" : `₹${prod.price}`}
                </span>
                <button className="bg-slate-100 text-slate-800 text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-blue-900 hover:text-white transition-colors">
                  Add to Cart
                </button>
              </div>
            </div>
      ...
      <section id="quote" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="bg-white rounded-3xl border border-slate-200 p-8 sm:p-12 shadow-2xl relative">
          <div className="absolute top-0 right-12 transform -translate-y-1/2 bg-amber-500 text-slate-950 font-bold text-xs uppercase px-4 py-1.5 rounded-full shadow">
            Fast Response Guarantee
          </div>
          <div className="text-center max-w-xl mx-auto mb-10">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight mb-2">Request A Custom Price Quote</h2>
            <p className="text-slate-500 text-sm">Fill out the fields below and an expert account engineer will compile custom matrix pricing sheets within 24 business hours.</p>
          </div>
          ...
          <form onSubmit={handleSubmitQuote} className="space-y-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <input required type="text" className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-900 transition-colors" placeholder="e.g. Alex Sangeetha" />
              </div>
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Email Address</label>
                <input required type="email" className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-900 transition-colors" placeholder="alex@brand.com" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                ...
                <select className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-900 transition-colors text-slate-700">
              ...
              <div>
                ...
                <input type="number" min="100" className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-900 transition-colors" placeholder="Minimum 100 pcs" />
              </div>
            </div>

            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-2">Project Details & Sizing Spec</label>
              <textarea rows={4} className="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-blue-900 transition-colors" placeholder="Describe layout, folds (center fold, miter fold), and dimensional lengths..."></textarea>
            </div>

            <button type="submit" className="w-full bg-blue-900 text-white font-extrabold tracking-wide py-4 rounded-xl text-sm uppercase hover:bg-blue-950 transition-all shadow-lg shadow-blue-900/20">
              Send Quote Request
            </button>
